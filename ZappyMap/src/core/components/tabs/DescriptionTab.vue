...
const itemClass =
  "flex gap-x-2 items-center text-gray-700 hover:text-emerald-600 transition-colors";
...
<template>
  <div v-if="restaurant" class="space-y-8 animate-in fade-in duration-500">
    ...
    <section class="pt-2" v-if="restaurant?.menus?.length > 0">
      <h2 class="font-bold text-xl text-gray-800 border-b border-gray-100 pb-3 mb-4">
        Carta
      </h2>
      ...
      >
        Disfruta de la gran variedad de platos que ofrece
        <span class="font-medium text-gray-700">{{ restaurant?.name }}</span> y vive el
        momento.
      </p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <div class="mb-4">
            ...
            <p v-if="menu.description" class="text-xs text-gray-500 line-clamp-2">
              {{ menu.description }}
            </p>
          </div>
    ...
    <section class="pt-2">
      <h2 class="font-bold text-xl text-gray-800 border-b border-gray-100 pb-3 mb-5">
        Detalles
      </h2>
      ...
      <dl class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6">
        <div v-if="restaurant.email" class="flex items-start gap-x-3">
          ...
          <dd class="min-w-0 flex-1">
            <span class="block text-xs text-gray-400 uppercase tracking-wider font-bold"
              >Email</span
            >
            <a
        ...
        <div v-if="restaurant.type_food" class="flex items-start gap-x-3">
          ...
          <dd class="flex-1">
            <span class="block text-xs text-gray-400 uppercase tracking-wider font-bold"
              >Cocina</span
            >
            <span class="text-gray-700 capitalize text-sm md:text-base">{{
              restaurant.type_food
            }}</span>
          </dd>
        </div>
    ...
    <section class="pt-2">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6">
        <div class="flex items-start gap-x-3">
          ...
          <div class="min-w-0 flex-1">
            <span
              class="block text-xs text-gray-500 uppercase tracking-widest font-bold mb-2"
            >
              Tarjetas
            </span>
            ...
            <div class="flex flex-wrap gap-3">
              <div
                v-for="(method, index) in restaurant?.payment_methods"
                :key="index"
        ...
        <div class="flex items-start gap-x-3">
          ...
          <div class="flex-1">
            <span
              class="block text-xs text-gray-400 uppercase tracking-wider font-bold mb-2"
            >
